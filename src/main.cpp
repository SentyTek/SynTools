...
void printHelp() {
    ...
    std::cout << "  shadermeta | sm    Generate shader metadata for an entire "
                 ...
                 "directory, containing all shaders.\n";
    std::cout << "\nOptions:\n";
...
int main(int argc, char** argv) {
    ...
    if (std::string(argv[1]) == "--version" || std::string(argv[1]) == "-v") {
        std::cout << "Syngine Tools Version: " << SYNGINE_TOOL_VERSION
                  << std::endl;
        return 0;
    }

    std::string command = argv[1];
    if (command == "shader" || command == "shaders" || command == "s") {
        ...
        bool success = SynTools::ShaderCompiler::CompileShader(
            fragmentPath, vertexPath, outputPath, options);
        if (!success) {
            std::cerr << "Shader compilation failed." << std::endl;
            return 1;
        }
        std::cout << "Shader compiled successfully." << std::endl;
    } else if (command == "pack" || command == "package" ||
               command == "packager" || command == "p") {
        ...
        SynTools::AssetPackager packager;
        bool                    success = packager.CreateFileBundle(
            files, outputPath, std::string(argv[0]), options);
        if (!success) {
        ...
        bool success =
            SynTools::ShaderMeta::GenerateShaderMeta(shaderPath, options);
        if (!success) {
            std::cerr << "Shader metadata generation failed." << std::endl;
            return 1;
        }
