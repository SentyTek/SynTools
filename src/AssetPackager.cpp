...
bool AssetPackager::CreateFileBundle(const std::vector<std::string>& files,
    ...
    pack.open(outputPath.c_str());
    auto result = pack.openFiles(entries);
    
    for (size_t i = 0; i < result.size(); i++) {
        auto& wt = *result[i];
        auto file = diskPaths[i];
        if (!wt->open(file, scl::OpenMode::READ, true)) {
            std::cout << "  Failed to add file: " << file.cstr() << std::endl;
            return false;
        }
    }
...
void AssetPackager::ValidatePackage(const std::string& packagePath) {
    ...
    for (const auto& entry : index) {
        ...
        totalOriginal += entry->m_original;
        std::cout << " - " << entry->m_file.cstr() << " (" << entry->m_size
                  << " / " << entry->m_original << " bytes) "
                  << (int)((double)entry->m_size / (double)entry->m_original * 100.0)
                  << "% compressed)" << std::endl;
    }
